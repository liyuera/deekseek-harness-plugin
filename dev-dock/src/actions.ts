...
import { detectEditors, KNOWN_EDITORS, PREFERRED_EDITORS } from './editors.ts'
 ...
 */
export function hasUniAppTraits(workspacePath: string): boolean {
 ...
 */
export function defaultEditorFor(workspacePath: string, installed: readonly string[]): string | undefined {
 ...
 */
export async function editorExecutablePath(
  scope: DesktopScope,
  facts: PlatformFacts,
  editorName: string,
 ...
 */
export async function resolveWorkspaceEditor(
  scope: DesktopScope,
  facts: PlatformFacts,
  workspace: DesktopWorkspace,
): Promise<{ ok: true; editor: string; path: string } | { ok: false; error: string }> {
 ...
 */
export async function openIdeFor(
  scope: DesktopScope,
  facts: PlatformFacts,
  workspace: DesktopWorkspace,
 ...
 * @returns ok, or an error message.
 */
export async function openTerminalFor(
  scope: DesktopScope,
  facts: PlatformFacts,
  workspace: DesktopWorkspace,
): Promise<DesktopActionResult> {
 ...
 */
export async function startWorkFor(
  scope: DesktopScope,
  facts: PlatformFacts,
  ...
  const items: Array<{ workspaceId: string; ok: boolean; error?: string }> = []
  let opened = 0
  let started = 0
  ...
  let lastEditorAppPath: string | undefined
  for (const workspace of workspaces) {
    ...
    const ideResult = ide.ok
      ...
      : { ok: false as const, error: ide.error }
    if (ide.ok) lastEditorAppPath = ide.path
    const terminal = await openTerminalFor(scope, facts, workspace)
    if (ideResult.ok) opened++
    if (terminal.ok) started++
    const item: { workspaceId: string; ok: boolean; error?: string } = {
 ...
 */
async function waitForAppProcess(facts: PlatformFacts, appPath: string): Promise<void> {
...
export function isKnownEditor(name: string): boolean {
  return KNOWN_EDITORS.includes(name as (typeof KNOWN_EDITORS)[number])
}
