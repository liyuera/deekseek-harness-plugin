...
export function scopeOf(scope: SettingsScope<DevDockSettings>): DevDockScope {
  return {
    get: () => scope.get(),
 ...
 * @returns the next id.
 */
export function nextProjectId(projects: readonly ProjectRecord[]): ProjectId {
  let max = 0
...
export function upsertProject(
  current: DevDockSettings,
  project: Omit<ProjectRecord, 'id' | 'createdAt'> & { id?: ProjectId },
): { next: DevDockSettings; stored: ProjectRecord } {
...
export function saveProjectTool(scope: DevDockScope) {
  return defineTool({
    name: 'dev-dock_save-project',
    description:
      'Save one analyzed frontend project into the devDock registry. Insert when the path is new; update overwrite when it already exists (keeps its id and createdAt). Call once per candidate after the AI analysis judged it a frontend project.',
    parameters: {
    ...
    async execute(args) {
      ...
      const record: Omit<ProjectRecord, 'id' | 'createdAt'> & { id?: ProjectId } = {
        path: args.path,
        name: args.name,
        type: args.type as ProjectRecord['type'],
        packageManager: args.packageManager as ProjectRecord['packageManager'],
        scripts: (args.scripts ?? {}) as Record<string, string>,
      }
      if (args.nodeVersion !== undefined) record.nodeVersion = args.nodeVersion
      if (args.buildCommand !== undefined) record.buildCommand = args.buildCommand
      if (args.alias !== undefined) record.alias = args.alias
...
export function listProjectsTool(scope: DevDockScope) {
  return defineTool({
    name: 'dev-dock_list-projects',
    description:
      'List all projects registered in the devDock registry: id, name, path, type, package manager, node version, scripts, build command, alias.',
    parameters: {},
    output: {
      schema: {
        type: 'array',
...
export function removeProjectTool(scope: DevDockScope) {
  return defineTool({
    name: 'dev-dock_remove-project',
    description:
      'Remove one project from the devDock registry. Only removes the plugin-managed configuration; never touches files on disk. Also removes the project from every quick-start plan.',
    parameters: {
    ...
    async execute(args) {
      ...
      if (result === null) return { removed: false, id: args.projectId }
...
function formatProjectList(projects: readonly Record<string, unknown>[]): string {
  if (projects.length === 0) return 'No projects registered.'
  return projects.map((p) => {
    const record = p as unknown as ProjectRecord
