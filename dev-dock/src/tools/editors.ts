...
export function listEditorsTool(scope: DevDockScope, facts: PlatformFacts) {
  return defineTool({
    name: 'dev-dock_list-editors',
    description:
      'Detect code editors installed on this machine (WebStorm, VS Code, IntelliJ IDEA, Cursor, Sublime Text, HBuilderX) and merge user-configured paths. Returns each editor with its detected path and/or manual path.',
    parameters: {},
    output: {
      schema: {
        type: 'array',
...
function formatEditors(editors: readonly EditorRecord[]): string {
