window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function SessionActionButton({ sessionId, useWorkspaces, useDevDockData, action, dataActions, t }) {
			...
			const okTimer = (0, react.useRef)(null);
			(0, react.useEffect)(() => () => {
			...
			const run = async () => {
				...
				setState("busy");
				setError("");
				const answer = action === "ide" ? await dataActions.openIde(workspace.workspaceId) : action === "terminal" ? await dataActions.openTerminal(workspace.workspaceId) : await dataActions.start(workspace.workspaceId);
				if (answer.ok) {
					setState("ok");
					if (okTimer.current !== null) clearTimeout(okTimer.current);
			...
			const glyph = () => {
				if (action === "ide") {
					if (iconFailed.ide === true) return (0, react_jsx_runtime.jsx)(DevIdeAppIcon, { size: 16 });
				...
				if (action === "terminal") {
					if (iconFailed.terminal === true) return (0, react_jsx_runtime.jsx)(DevTerminalAppIcon, { size: 16 });
					return (0, react_jsx_runtime.jsx)("img", {
						className: SessionActions_module_css_default.appIcon,
						src: terminalIconUrl,
						alt: "",
			...
			const title = state === "error" ? t("header.error", { error }) : label;
