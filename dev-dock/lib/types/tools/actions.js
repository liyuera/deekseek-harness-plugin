 ...
 */
export async function requireApproval(approval, exec, toolName, reason) {
...
export function openIdeTool(scope, facts, approval) {
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            if (project === undefined)
                return { ok: false, error: `project ${args.projectId} not found` };
...
export function openTerminalTool(scope, facts, approval) {
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            const allowed = await requireApproval(approval, exec, 'dev-dock_open-terminal', `Open terminal at ${project.name}${command === undefined ? '' : ` and run ${command}`}`);
            if (!allowed.allowed)
                return { ok: false, error: allowed.error };
...
export function quickStartTool(scope, facts, approval) {
    return defineTool({
